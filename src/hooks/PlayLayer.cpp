...
class BI_DLL $modify(BIPlayLayer, PlayLayer) {
    ...
    void onQuit(){
        ...
        GEODE_DESKTOP(if(Mod::get()->getSettingValue<bool>("auto-submit") && this->m_level->m_levelType == GJLevelType::Saved) BetterInfoScheduler::sharedState()->submitLevel(this->m_level));
        PlayLayer::onQuit();
    }
